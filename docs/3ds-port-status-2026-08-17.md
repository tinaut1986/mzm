...
- **Audio: petardeo — CAUSA RAÍZ ENCONTRADA Y ARREGLADA, PENDIENTE DE CONFIRMACIÓN POR OÍDO/EN HARDWARE REAL (rama `wip/3ds-port-audio`, 2026-08-19).** Ver secciones 6k-6q. `gMusicInfo.unk_10` (la posición de "cuánto ha consumido ya el DMA real") solo se actualizaba en `DMA2IntrCode()`, que nunca se invocaba en el port (no hay hardware DMA2 real en 3DS) — se quedaba congelada toda la partida, desacoplando la aritmética de `UpdateMusic()` (`src/audio.c:32-116`) de cuánto se reproducía de verdad. Arreglado en `src/agbmain.c`: se llama a `DMA2IntrCode()` según el consumo real del anillo NDSP (`Port_MzmAudio_RingReadIndex()`), una vez por cada 384 muestras (`PCM_DMA_BUF_SIZE/4`). **Verificado cuantitativamente contra una referencia de `mgba`: de 739 discontinuidades grandes a 6-11 (~98% de reducción), reproducible en varias ejecuciones, estable 150s sin crashear.** Sin verificar todavía por oído ni en hardware real (el usuario estaba fuera del PC durante el fix) — es la tarea pendiente más importante de la próxima sesión. Si suena mal pese a la mejora medida, revisar si "384 muestras por incremento" es exactamente la cadencia correcta (alternativa a probar: ligarlo a `gMusicInfo.unk_C`=14 en vez de a `PCM_DMA_BUF_SIZE`/4).
