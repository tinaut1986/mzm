...
- **Audio: petardeo residual (rama `wip/3ds-port-audio`, 2026-08-19).** Ver secciones 6k-6m para el detalle completo. Seis bugs reales de audio ya arreglados y confirmados en hardware/Azahar (silencio→sonido real), pero el sonido sigue con petardeo (ruido de banda ancha continuo en el espectrograma, superpuesto a contenido tonal real) sin limpiar del todo. Confirmado en 6l que NO es un problema de tamaño/cantidad de buffers NDSP (se probó reducirlos y reordenar el colchón de underrun, y empeoró). Confirmado en 6m que tampoco es (solo) el bug de `lr` corrompido en GOTO/PATT (arreglado ahí, el espectrograma apenas cambia). **Importante para la próxima sesión:** `tools/run_azahar_test.sh` estuvo decodificando el volcado de audio como `u8` (sin signo) en vez de `s8` (con signo, el formato confirmado correcto) durante buena parte de esta sesión — ya arreglado, pero cualquier captura/espectrograma de ANTES de este fix en este chat puede verse más ruidosa de lo que realmente es. Se resolvieron dos punteros de muestra sin traducir más en `unk_4e10` (mismo patrón que los bugs de 6k, ver 6m) sin que cambiara el espectrograma en la prueba de 90s — puede que esa ruta de código no se ejercite con la música probada, o que el bug real esté en otro sitio. **Siguiente paso recomendado:** auditar `asm/soundcode.s` (el mezclador final, `CallSoundCodeB`/`CallSoundCodeC`) función por función con el mismo criterio que hoy en `audio_internal.s` — no se ha revisado todavía con el mismo nivel de detalle.
