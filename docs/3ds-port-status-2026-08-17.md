...
- **Audio: petardeo residual, sin diagnosticar más allá de la sección 6k (rama `wip/3ds-port-audio`, 2026-08-19).** Ver sección 6k para el detalle completo. Seis bugs reales de audio ya arreglados y confirmados en hardware/Azahar (silencio→sonido real), pero el sonido sigue con petardeo sin limpiar del todo — se confirmó en la sesión 6l que NO es un problema de tamaño/cantidad de buffers NDSP (se probó reducirlos y reordenar el colchón de underrun, y empeoró).
- **FPS: la cifra de "~20 FPS" de la sección 6k necesita remedirse con un build limpio (`make clean && make`) antes de seguir diagnosticándola.** Ver sección 6l — se encontró que un build incremental obsoleto (objetos compilados cuando `PORT_VERBOSE_FRAME_LOG` estaba activo, sin recompilar tras quitar el flag) puede por sí solo hacer bajar los FPS medidos en Azahar de ~29 a ~10-11. No confirmado todavía si la regresión original en hardware real de 6k es el mismo espejismo o un problema real independiente del movimiento a Core 1.
- **Nuevo crash sin diagnosticar (2026-08-19, sección 6l): `UndefinedInstruction` en la intro** tras dejar correr el juego más tiempo del habitual en las pruebas. `pc` salta a `.rodata` (no a código), `lr` apunta a `port_resolve_addr`, `sp` vuelve a aliasar el rango de direcciones GBA ROM (mismo fenómeno que 6i). Huele a un puntero a función sin resolver en algún sitio del camino de la intro, mismo patrón que los bugs de audio de 6k pero sin localizar todavía dónde.
