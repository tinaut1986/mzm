...
- **Audio: petardeo residual (rama `wip/3ds-port-audio`, 2026-08-19).** Ver secciones 6k-6o para el detalle completo. Seis bugs reales de audio ya arreglados y confirmados en hardware/Azahar (silencio→sonido real), pero el sonido sigue con petardeo. Descartado: tamaño/cantidad de buffers NDSP (6l, empeoró al tocarlo), el bug de `lr` corrompido en GOTO/PATT (6m, arreglado pero no cambia el audio), formato de decodificación del script de diagnóstico (ya corregido a `s8`), varios puntos de `asm/soundcode.s` (6n: DMA1 crudo es código muerto; `pData`/`pSize` se derivan bien de `pSample1` ya resuelto), y que el puente de captura del port concatene mal direcciones (6o: `dest` es siempre el mismo buffer fijo, sin huecos). **Confirmado cuantitativamente en 6o (comparación muestra-a-muestra contra una grabación de referencia de `mgba`): el petardeo es real (739 discontinuidades grandes en el port vs 0 en la referencia) y se concentra de forma no aleatoria justo en los límites de cada bloque de mezcla de 432 muestras del motor** (`n=432` en los logs). Siguiente paso más prometedor: revisar en `src/agbmain.c` (cerca de `UpdateAudio()`) si la cadencia con la que se invoca al motor de audio cada frame coincide con lo que el diseño de doble buffer original asume sobre el ritmo de consumo de DMA en hardware real — no seguir auditando asm sin evidencia dirigida.
