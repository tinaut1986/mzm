...
- **Audio: petardeo residual (rama `wip/3ds-port-audio`, 2026-08-19).** Ver secciones 6k-6p para el detalle completo. Seis bugs reales de audio ya arreglados y confirmados en hardware/Azahar (silencio→sonido real), pero el sonido sigue con petardeo. Descartado: tamaño/cantidad de buffers NDSP (6l), el bug de `lr` corrompido en GOTO/PATT (6m, arreglado pero no cambia el audio), formato de decodificación del script de diagnóstico (corregido a `s8`), varios puntos de `asm/soundcode.s` (6n), que el puente de captura concatene mal direcciones (6o: `dest` es fijo, sin huecos), y un crossfade de 8 muestras en los límites de bloque (6p, probado offline, sin efecto). **Confirmado cuantitativamente (6o, comparación muestra-a-muestra contra `mgba`): 739 discontinuidades grandes en el port vs 0 en la referencia.** **Localización más precisa (6p): el salto se concentra en el desplazamiento 383 dentro de cada bloque de 432 muestras (~38% de los casos), no en el límite del bloque (431)** — apunta a un punto fijo dentro del propio `SoundCodeC`, probablemente relacionado con las constantes de `SetupSoundTransfer` (`unk_C`/`unk_D`/`unk_E`, sección 6) que en el hardware real re-sincronizarían con el DMA cada vblank. Siguiente paso recomendado: loguear esas constantes en tiempo de ejecución y comparar contra 384/48, luego localizar en `asm/soundcode.s` qué ocurre exactamente ahí dentro del bucle de `SoundCodeC`. **Trampa de medición a evitar:** cualquier instrumentación debe agrupar la E/S a fichero (una apertura por llamada), nunca por muestra — abrir/cerrar el fichero 432 veces por frame provoca FPS 0 instantáneo y no es un bug real del fix que se esté probando.
