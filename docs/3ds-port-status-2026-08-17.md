...
**Deliberadamente NO se ha tocado más código de audio esta sesión** más allá de los dos resolves de `unk_4e10` — después de que el tuning de buffers de la sección 6l resultara ser una regresión, se prioriza no volver a aplicar cambios especulativos sin evidencia sólida de que ayudan. El siguiente paso más útil es probablemente comparar audio muestra-a-muestra contra una referencia real, no seguir leyendo asm a ciegas.

## 7. Pendientes y Siguientes Pasos

**Bugs abiertos pendientes de investigar (reportados por el usuario, aún SIN resolver):**
- **Audio: petardeo residual (rama `wip/3ds-port-audio`, 2026-08-19).** Ver secciones 6k-6n para el detalle completo. Seis bugs reales de audio ya arreglados y confirmados en hardware/Azahar (silencio→sonido real), pero el sonido sigue con petardeo sin limpiar del todo. Descartado: tamaño/cantidad de buffers NDSP (6l, empeoró al tocarlo), el bug de `lr` corrompido en GOTO/PATT (6m, arreglado pero no cambia el audio), formato de decodificación del script de diagnóstico (ya corregido a `s8`), y varios puntos de `asm/soundcode.s` auditados en 6n (DMA1 crudo es código muerto sin llamadores; `pData`/`pSize` del mezclador se derivan correctamente de `pSample1` ya resuelto). **Giro de diagnóstico importante en 6n: una forma de onda real (no solo espectrograma) de música en juego se ve musicalmente coherente, no como basura de memoria corrupta** — pone en duda que "puntero sin resolver" siga siendo la explicación principal. Hipótesis para continuar, sin confirmar: bug de escala/overflow en la mezcla de canales, diferencia de fidelidad analógica-vs-digital inherente (GBA real filtra el PCM crudo vía su DAC, NDSP no), o discontinuidades residuales en los límites de buffer. Siguiente paso recomendado: comparar audio muestra-a-muestra contra una referencia real (ej. `mgba`), no seguir auditando asm sin evidencia dirigida.
