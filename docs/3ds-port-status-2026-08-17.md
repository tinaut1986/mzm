...
**Próximos pasos recomendados (actualizado tras la sesión de investigación en Linux, ver 6k):**
1. ~~Acotar el frame exacto de desaparición con precisión de 1 frame~~ — HECHO, ya en esta misma sesión: con el escaneo de `gSpriteData` cada tick (en vez de cada 30) se localizó el desvanecimiento con precisión de 1 tick exacto (`"Deorem VANISHED at tick=1501"`, sano un tick antes en `tick=1500`). Localizar el frame exacto no es ya el cuello de botella — lo es identificar QUÉ escritura concreta borra `SPRITE_STATUS_EXISTS`/corrompe el slot, y para eso hace falta un watchpoint de memoria real (gdb), que es precisamente lo que motivó el intento de reproducirlo en Linux (6k, pausado sin éxito).
2. **Recomendado como primer paso al retomar:** o bien (a) continuar el esfuerzo de Linux — el hueco conocido es que `RoomReset()` recalcula `gCurrentArea`/`gCurrentRoom` desde `gLastDoorUsed` y hace falta averiguar qué estado exacto de un arranque real falta replicar antes de forzar `GM_INGAME` (ver 6k para el punto exacto donde se dejó), o (b) seguir por 3DS+FTP con los puntos 3-4 de abajo.
3. Revisar `Port_GbaTiming_ThreadMain` (`platform/3ds/source/platform_3ds_minimal.c`) para confirmar si toca memoria compartida con el estado de sprites sin lock — nota: la sección 6k encontró evidencia de que el frame de desaparición varía por ejecución, lo cual sigue siendo compatible tanto con una condición de carrera real como con una dependencia de la disposición de la pila (ver el bug de `Platform3DS_IsActiveStackAddress` en Linux, 6k/6l del día anterior) — no descartar esta segunda opción.
4. Probar el mismo experimento de aislar-por-desactivación con `DeoremSpriteDebrisSpawn`/`ParticleSet`/`SpriteDebrisInit` (los únicos efectos que quedan sin descartar dentro del primer frame de `DeoremSpawnGoingDown`), ya que el audio queda descartado pero esas llamadas no se han aislado todavía.
5. Considerar log/verificación de `SpriteUtilSamusAndSpriteCollision` (colisión Samus-sprites, se llama una vez por frame en `SpriteUpdate` antes del bucle principal) como sospechoso de "matar" a Deorem por una colisión mal calculada, aunque no se ha visto pasar por ninguna pose de muerte — comprobar si existe algún camino que ponga `status = 0` directamente sin pasar por el pose de destrucción.
...
Motivación: cazar la causa exacta con `gdb`/sanitizers en el PC en vez de iterar por FTP contra la 3DS real. El port de Linux (`platform/linux/`) comparte el mismo `port_gba_mem.c`/mecanismo de traducción de direcciones GBA↔host que la 3DS (compila con `-DTMC_3DS` también).

**Bugs reales encontrados y corregidos en el Makefile/infraestructura de Linux:**
1. Faltaba `-I$(ROOT)` en `CPPFLAGS` — sin él, `#include "port/generated/empty_datatypes_rom.h"` no se encontraba.
2. Faltaban 4 archivos fuente que sí compila 3DS: `src/data/color_fading_data.c`, `src/data/cutscenes/cutscenes_data.c`, `src/data/in_game_cutscene_data.c`, `src/data/empty_datatypes.c`.
3. Faltaba el equivalente de `ewram_symbols.ld` — solucionado inyectando `-Wl,--defsym,sym=expr` generado con `sed`.
4. **Bug de arquitectura:** con `-m64`, el juego crasheaba al leer `pHeader` basura. Causa: `struct SoundEntry` se lee como bytes crudos de ROM, por lo que el tamaño del puntero `const u8* pHeader` desalinea toda la estructura. **Fix: compilar el port de Linux en 32 bits (`-m32`).**

**Estado del renderizado:** el port de Linux tiene ventana y teclado X11 completamente escritos, pero el framebuffer nunca se pinta en la ventana — a día de hoy **no es jugable visualmente**.

**Decisión (2026-08-18):** pausado por esta noche. El objetivo no se llegó a cumplir, pero los fixes de Makefile/32-bit son reales y quedan aplicados.

## 6m. Solución definitiva al bug de desaparición de Deorem (2026-08-18)

**Causa raíz encontrada:** Cuando `gSpriteDebris` se llena, `SpriteDebrisInit()` (`src/sprite_debris.c`) buscaba reciclar mediante un puntero que, tras salir del bucle, siempre apuntaba **un elemento más allá del final del array `gSpriteDebris`**. En el port nativo de 3DS, esta escritura fuera de límites sobreescribía `gCurrentSprite`, borrando a Deorem en mitad del combate.

**Solución aplicada:** Se corrigió la indexación en `SpriteDebrisInit()` (`src/sprite_debris.c`) para reciclar correctamente el slot más antiguo sin sobrepasar el array. Con esto, el combate contra Deorem funciona al 100% sin desapariciones.

## 6n. Sesión de audio 2026-08-19: seis bugs reales encontrados y arreglados, petardeo reducido pero persiste, nueva regresión de FPS

**Metodología:** Análisis de espectrogramas de `mzm_audio_dump.bin`.
**Seis bugs encontrados:**
1. `InitTrack` no resolvía `pHeader` (`asm/audio_internal.s`).
2. `AudioCommand_Voice` no resolvía `pSample` (`src/audio.c`).
3. `unk_2030` desreferenciaba `pSound->pVariables` sin comprobar NULL.
4. `QueueSound` leían `pHeader[0]`/`pHeader[2]` directamente.
5. `gUnk_300376C` dimensionado erróneamente (`src/globals1.c`).
6. Puente NDSP trataba PCM con signo como sin signo (`port/port_mzm_audio_glue.c`).

## 6o. Sesión de continuación 2026-08-19: falsa pista de la FPS, nuevo crash real en la intro, y automatización de pruebas en Azahar

**Hallazgo importante — la "regresión de FPS a ~20" de la sección 6n estaba contaminada por un build incremental obsoleto:** un `make clean && make` limpio hizo que los FPS subieran de ~10-11 a **~29** en Azahar. **Conclusión: cualquier medición de FPS debe hacerse SIEMPRE con `make clean` de por medio.**

## 6p. Root cause real del crash de la intro Y de gran parte de la regresión de FPS: `lr` pisado por `bl port_resolve_addr` (2026-08-19)

**Causa encontrada en `asm/audio_internal.s`:** `AudioCommand_Goto` y `AudioCommand_PatternPlay` son funciones hoja; el fix de traducción de punteros insertó `bl port_resolve_addr` sin guardar `lr`, sobrescribiéndolo. El `bx lr` final devolvía a un sitio corrupto o saltaba a NULL.

**Fix:** Guardar/restaurar `lr` pasando por un registro callee-saved libre (`r4`).
**Resultado:** **120+ segundos sin crashear**, FPS estables en ~55.

## 6q. Continuando la caza del petardeo: auditoría de `soundcode.s` y giro importante en el diagnóstico (2026-08-19)

**Auditoría:** `SoundCodeA` (`asm/soundcode.s`) correcto, copiado a IWRAM deshabilitado para 3DS. La forma de onda es coherente musicalmente, descartando que el petardeo principal sea corrupción de memoria.

## 6r. Diagnóstico cuantitativo del petardeo (2026-08-19)

Comparación contra mGBA de referencia:
```
...
## 6w. Causa raíz de las pistas ausentes de Direct Sound encontrada y RESUELTA: `MidiKey2Freq` hardcodeaba 261 Hz (2026-08-20)

**Contexto:** tras descartar el PSG en 6v, se auditó la ruta de Direct Sound (`asm/audio_internal.s`, `src/audio.c`, `port/port_bios.c`, `src/audio_wrappers.c`, `src/agbmain.c`) en busca de por qué faltaban voces melódicas y pistas de la música.

**Los cuatro hallazgos y correcciones aplicadas:**

1. **La causa raíz principal — `MidiKey2Freq` en `port/port_bios.c` ignoraba la frecuencia de las muestras:**
   - La implementación de BIOS `MidiKey2Freq` tenía un placeholder que ignoraba el parámetro `waveData` y hardcodeaba `double freq = 261.626 /* middle C */ * exp2(...)`.
   - En GBA, cada cabecera de muestra almacena su frecuencia de muestreo base en `waveData[1]` en unidades de `Hz * 1024` (ej. 6.69 kHz, 13.38 kHz, 26.76 kHz).
   - Al devolver ~261 Hz para todas las muestras en vez de su frecuencia real, el paso de fase (`unk_1C`) en el mezclador `SoundCodeA` resultaba **entre 25 y 100 veces más lento de lo debido**.
   - Cada muestra se reproducía **entre 5 y 7 octavas más grave y a 1/50 de su velocidad**, quedando en el registro subsónico inaudible. Arreglado calculando `freq = ((double)baseFreq * exp2(semitones / 12.0)) / 1024.0`.

2. **Resolución de tabla de notas (KeyMap) para instrumentos multi-muestra (`channel == 0x40`) en `src/audio.c`:**
   - En `AudioCommand_Voice`, para `channel == 0x40`, `pVoice->envelope` no es una envolvente normal sino un puntero crudo de ROM (`0x08xxxxxx`) a una tabla de 128 bytes que mapea notas MIDI a sub-voces.
   - Se copiaba sin `GBA_RESOLVE`, por lo que `unk_4e10` en asm leía memoria basura en 3DS al desreferenciar `envelope2`. Arreglado resolviendo el puntero antes de almacenarlo en `pVariables->envelope2`.

3. **Eliminación del límite de tiempo por scanline de GBA (`REG_VCOUNT`) en `src/audio_wrappers.c`:**
   - En `InitializeAudio`, `gMusicInfo.unk_9` se inicializaba a `0x64` (100 scanlines). En GBA servía para que el audio no consumiera demasiado tiempo de CPU.
   - En 3DS, con el hilo de timing de `REG_VCOUNT` desacoplado, la condición `vcount >= var_0` en `UpdateMusic` provocaba un `break` prematuro en el bucle de mezcla de canales, descartando canales activos. Configurado `gMusicInfo.unk_9 = 0` para `PORT_NATIVE`/`TMC_3DS`.

4. **Corrección de cadencia en `DMA2IntrCode` y array bounds en `src/agbmain.c` / `src/audio_wrappers.c`:**
   - Cada paso de `gMusicInfo.unk_10` en `DMA2IntrCode()` representa un bloque de **16 muestras** (no 384). Se ajustó el umbral de acumulación de 384u a 16u en `agbmain.c`.
...
- **Gráficos animados (`src/animated_graphics.c:307`):** auditoría de `pData->pGraphics` sin `GBA_RESOLVE` para animaciones de tiles (agua, lava, etc.).
...
- **RESUELTO (2026-08-19, sección 6p): Crash determinista de la intro y recuperación de FPS (~55 FPS).** Era el bug de `lr` corrompido: `bl port_resolve_addr` insertado en `AudioCommand_Goto`/`AudioCommand_PatternPlay` (`asm/audio_internal.s`) sin guardar `lr`. Con el fix: intro completa y juego estable.
- **RESUELTO (2026-08-18, sección 6m): Desaparición de Deorem.** Causa: `SpriteDebrisInit()` (`src/sprite_debris.c`) indexaba un elemento fuera de límites sobreescribiendo `gCurrentSprite`. Corregido y combate 100% funcional.
- **RESUELTO (2026-08-18, sección 6l): Infraestructura Linux y corrección de 32-bit.**
- **RESUELTO (2026-08-18, secciones 6i/6j): Guardado en SRAM y recogida de tanque de misiles.**

**Siguientes tareas y mejoras recomendadas:**

1. **Opciones de pantalla / Aspect Ratio y Segunda Pantalla:**
...
2. Subir el CIA por FTP (FBI corriendo en la consola, puerto 5000 — la IP
   ...
   `192.168.1.133` como `.138` (`.138` funcionó durante toda la sesión del
   2026-08-18); probar ambas si una falla):
   ```
