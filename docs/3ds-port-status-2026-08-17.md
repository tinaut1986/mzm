...
- **Audio PSG sin implementar — causa de que solo suenen algunas pistas de la música (sección 6t, 2026-08-19).** Las 4 voces PSG de la GBA (`gPsgSounds[4]`: 2 cuadradas, onda y ruido) no se sintetizan en el port: el motor escribe sus registros, esas escrituras van a `gIoMem` y nadie las lee nunca. No es un bug sino funcionalidad que falta; es, con diferencia, lo que más separa ahora mismo el audio del port del de la GBA real. Ver 6t para el plan de implementación y para por qué conviene mirar antes el APU de `nes_metroid/`.
