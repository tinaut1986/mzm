...
- **Audio PSG — IMPLEMENTADO Y CON DOS BUGS ARREGLADOS, PENDIENTE DE PRUEBA POR OÍDO EN HARDWARE (secciones 6t y 6u, 2026-08-19).** Las 4 voces PSG ya se sintetizan por software (`port/port_psg_synth.c`, con tests unitarios deterministas en `make test-psg`). Además se encontró la causa real de que estuvieran mudas incluso con sintetizador: un "arreglo" de la sesión 6k rompió el aliasing `gUnk_300376C`/`gPsgSounds`, que era load-bearing, así que las notas se guardaban donde nadie las lee (ver 6u, con evidencia de runtime: paneo `0x00` antes, `0x88` después). **La CIA está compilada pero NO subida** (la consola no respondía por FTP al terminar la sesión). Falta confirmar de oído si ahora suenan más pistas; en 90s de intro solo se observó actividad en la voz de ruido, lo que puede ser normal para esa música o indicar que las cuadradas siguen sin recibir notas.
